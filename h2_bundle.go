...
//   $ bundle -o=h2_bundle.go -prefix=http2 -tags=nethttpomithttp2 ./
...
func http2configFromServer(h1 *http.Server, h2 *http2Server) http2http2Config {
...
func http2fillNetHTTPServerConfig(conf *http2http2Config, srv *http.Server) {}

func http2fillNetHTTPTransportConfig(conf *http2http2Config, tr *http.Transport) {}
...
func (fr *http2Framer) readMetaFrame(cont http2continuable) (http2Frame, error) {
	...
	for {
		...
		if int64(len(frag)) > int64(2*remainSize) {
			...
			return hc, http2ConnectionError(http2ErrCodeProtocol)
		}
		...
		if _, err := hdec.Write(frag); err != nil {
			return hc, http2ConnectionError(http2ErrCodeCompression)
		}
...
// will be cancelled.
func (pr *http2PushedRequest) ReadResponse(ctx context.Context) (*http.Response, error) {
	cs := pr.pushedStream

	handleResponseHeaders := func() (*http.Response, error) {
		res := cs.res
		res.Request = pr.Promise
		res.TLS = cs.cc.tlsState
		return res, nil
	}
	...
	case <-cs.abort:
		select {
		case <-cs.respHeaderRecv:
			// See cc.RoundTrip docs about golang.org/issue/49645
			return handleResponseHeaders()
		default:
...
func (pr *http2PushedRequest) Cancel() {
	pr.pushedStream.abortStream(http2errRequestCanceled)
}

func http2pushedRequestToHTTPRequest(mppf *http2MetaPushPromiseFrame) (*http.Request, error) {
...
func http2handlePushEarlyReturnCancel(pushHandler http2PushHandler, pushedRequest *http2PushedRequest) {
	...
	cs.cleanupPushPromise(err)
	// FIXME: do not send RST_STREAM if the stream has already been closed
	// pr.pushedStream.cc.writeStreamReset(pr.pushedStream.ID, ErrCodeCancel, err)
}
...
func http2checkValidPushPromiseRequestHeaders(h http.Header) error {
...
type http2ClientConn struct {
	...
	flow             http2outflow // our conn-level flow control quota (cs.flow is per stream)
...
func (t *http2Transport) newClientConn(c net.Conn, addr string, singleUse bool) (*http2ClientConn, error) {
	...
	if max := conf.MaxReadFrameSize; max != 0 {
...
func (cc *http2ClientConn) roundTrip(req *http.Request, streamf func(*http2clientStream)) (*http.Response, error) {
	...
	cs := &http2clientStream{
		...
		reqURL:               req.URL,
		reqHeaderCloned:      http2cloneHeader(req.Header),
...
func (cs *http2clientStream) cleanupPushPromise(err error) {
	cs.sentEndStream = true
	cs.sentHeaders = true
	cs.cleanupWriteRequest(err)
}
...
func (rl *http2clientConnReadLoop) processPushPromise(f *http2MetaPushPromiseFrame) error {
	...
	if cs == nil || cs.abortErr != nil || cs.readClosed {
		return http2ConnectionError(http2ErrCodeProtocol)
	}
	...
	rl.cc.addStreamLocked(pushedStream)
	rl.cc.mu.Unlock()

	pushedReq, err := http2pushedRequestToHTTPRequest(f)
	if err != nil {
		rl.endStreamError(cs, http2StreamError{f.StreamID, http2ErrCodeProtocol, err})
		return nil
	}
	...
	pr := &http2PushedRequest{
		Promise:               pushedReq,
		OriginalRequestURL:    cs.reqURL,
		OriginalRequestHeader: http2cloneHeader(cs.reqHeaderCloned),
