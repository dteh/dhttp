...
func testClientConnReserveAll(t testing.TB, mode testMode) {
...
func testClientConnReserveParallel(t testing.TB, mode testMode) {
...
func testClientConnReserveRelease(t testing.TB, mode testMode) {
	_, cc := newClientConnTest(t, mode, nil, optFakeNet, func(s *http.Server) {
		s.HTTP2 = &http.HTTP2Config{
			MaxConcurrentStreams: 3,
		}
...
func TestClientConnReserveAndConsume(t *testing.T) {
	for _, test := range []struct {
		name     string
		consume  func(t testing.TB, cc *http.ClientConn, mode testMode)
...
func testClientConnRoundTripBlocks(t testing.TB, mode testMode) {
